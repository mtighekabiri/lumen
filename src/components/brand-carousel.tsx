      ...
      <div className="relative">
        ...
        <div className="flex animate-scroll">
          {duplicatedBrands.map((brand, index) => (
            <div
              key={`${brand}-${index}`}
              className="flex-shrink-0 mx-4 bg-gray-50 dark:bg-gray-800 rounded-lg p-4 flex items-center justify-center h-16 w-32 border border-gray-200 dark:border-gray-700"
            >
              <Image
                src={`/brands/${brand}`}
                alt={brand.replace(/\.[^/.]+$/, "")}
